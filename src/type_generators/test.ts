...
async function generateMethodParams(schemaUrl: string): Promise<string> {
  ...
  function schemaToTsType(schema: any, visited = new Set()): string {
    ...
    // Handle allOf as intersection of types
    if (schema.allOf && Array.isArray(schema.allOf)) {
      const types = schema.allOf.map((subschema: any) => schemaToTsType(subschema, visited))
      return types.join(' & ')
    }
    ...
    if (schema.type === 'object' || schema.properties) {
      ...
      const props = Object.entries(properties)
        .map(([key, propSchema]: [string, any]) => {
          const tsType = schemaToTsType(propSchema, visited)
          const optionalFlag = requiredKeys.has(key) ? '' : '?'
          return `${key}${optionalFlag}: ${tsType}`
        })
        .join('; ')
  ...
  for (const [methodName, pathData] of Object.entries(schema.paths || {})) {
    ...
    methodEntries.push(`  '${methodName}': ${paramsType}`)
  }

  return `type MethodToParams = {\n${methodEntries.join('\n  ')}\n}`
}
...
;(async () => {
  try {
    ...
    console.log('✅ Generated MethodToParams object type')
